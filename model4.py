...
def pr(weather):
	...
	print("\ 1)altitude and distance are range value")
	print("\ 2)season:")
	print(weather)
	print("\ 3)exclude same station rental/return")
	print("\--------------------------------------")
	df = pd.read_csv('./modelling_9_' + weather + '_wo_same.csv')
...
def pr_predict(weather, x, y):
	...
	plt.savefig(weather + "_pr_model4.png")
	plt.clf()
	plt.cla()
	plt.close()
