...
def pr_plot(predictions, weather, model, x_test, y_test):
	...
	plt.savefig(weather + str(model) + "_1model.png")
	plt.clf()
	plt.cla()
	plt.close()
...
#model numbers
summerWinterModels = [1, 3, 7, 9]
allSeasonsModels = [2, 8]
...
for w in weather:
	...
	for m in summerWinterModels:
		if m == 1:
			...
			print("\ 2)altitude and distance are real value")
			print("\ 3)include a loop feature")
			formula ="""daily_usage~C(dayofweek)+high_real+low_real+perci_real+f_station_rack+t_station_rack+dis_range*alti_range+C(loop)"""
